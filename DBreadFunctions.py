...
def readparams(paramsetid, cnxn, table, log_file):
    ...
    sqlstr = 'SELECT CRITICALTEMP, BASETEMP, LIQUIDWATER, DELAYTIME, REFREEZING FROM ' + table + \
             ' WHERE IDPARAMSET = ?'
    curs = cnxn.cursor()
    try:
    ...
    refreezing = tmp[0][4]
    curs.close()
    return crtemp, basetemp, liquidw, delaytime, refreezing
...
def readsnowparams(paramsetid, basincod, cnxn, table, log_file):
    ...
    sqlstr = 'SELECT RMF, CMFMIN/2 + CMFMAX/2, ERF, ALBSNOW, ALBGLAC, BETA2, NMF, GAMMAH FROM ' + table + \
    ...
    gammah = tmp[0][7]
    curs.close()
    return rmf, cmf, erf, albsnow, albglac, beta2, nmf, gammah
